...
/* Ad-soyaddan kurumsal e-posta üretir: "Hamide Konuk" -> hamide.konuk@...
   RASTGELE EK KULLANILMAZ. Daha önce SokakYonetim ve TeskilatYonetim
   `${slug(ad)}.${Math.random()...}@` üretiyordu; her çağrı farklı adres verdiği
   için aynı kişi ikinci kez atandığında mevcut hesabı bulunamıyor ve mükerrer
   hesap açılıyordu (7 kişide oldu, biri bölgesini göremedi). Adres artık
   deterministik; çakışma olursa sunucu tarafı ya mevcut hesabı kullanır ya da
   sonuna sayı ekler. */
export function epostaUret(adSoyad, alan = "akpartibasaksehir.com") {
  const slug = (t) => (t || "").toLocaleLowerCase("tr")
    .replace(/ı/g, "i").replace(/ş/g, "s").replace(/ğ/g, "g")
    .replace(/ü/g, "u").replace(/ö/g, "o").replace(/ç/g, "c")
    .replace(/[^a-z0-9]/g, "");
  const parcalar = String(adSoyad || "").trim().split(/\s+/).filter(Boolean);
  const soyad = parcalar.length > 1 ? slug(parcalar.pop()) : "";
  const ad = slug(parcalar.join(" "));
  const yerel = [ad, soyad].filter(Boolean).join(".") || "uye";
  return `${yerel}@${alan}`;
}
