...
export default function SokakYonetim({ grup, userId, duzenle }) {
  ...
  async function sil(row) {
    const hesapVar = !!row.profil_id;
    if (!confirm(`${row.ad_soyad} ekipten çıkarılsın mı?${hesapVar ? "\n(Giriş hesabı da silinecek — bir daha giremez.)" : ""}`)) return;
    try {
      // Giriş hesabı varsa önce auth kullanıcısını sil (service_role → hesap-ekle DELETE)
      if (hesapVar) {
        const { data: { session } } = await supabase.auth.getSession();
        const r = await fetch("/api/hesap-ekle", {
          method: "DELETE",
          headers: { "Content-Type": "application/json", authorization: `Bearer ${session.access_token}` },
          body: JSON.stringify({ id: row.profil_id }),
        });
        const j = await r.json();
        if (!r.ok) throw new Error(j.error || "hesap silinemedi");
      }
      // Ekip satırını sil
      const { error } = await supabase.from("sokak_ekip").delete().eq("id", row.id);
      if (error) throw error;
      await yukle();
    } catch (e) {
  ...
  const yeterli = (uyeler?.length || 0) >= 3;
    ...
    <div className="panel" style={{ marginBottom: 14 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
        ...
        <span style={{ fontSize: 12, color: yeterli ? "#15803d" : "#b45309", background: yeterli ? "#e7f6ec" : "#fef3c7", border: `1px solid ${yeterli ? "#a7e3bd" : "#fde68a"}`, padding: "2px 9px", borderRadius: 999, fontWeight: 700 }}>
          {uyeler == null ? "…" : `${uyeler.length} üye`}{!yeterli && " · en az 3 gerekli"}
        </span>
        ...
        uyeler.length === 0
          ...
          : uyeler.map((u) => (
            <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, marginBottom: 5 }}>
              <span style={{ fontSize: 10.5, fontWeight: 700, background: "#eef2ff", color: "#3730a3", padding: "1px 7px", borderRadius: 999 }}>SOKAK YÖNETİMİ</span>
              <b>{u.ad_soyad || "—"}</b>
